    ...
    it("tests the whole flow", async () => {
        ...
        const tech2UserDetails: any = {
         ...
         expect((await request(app.getHttpServer())
            .post("/user")
            .send({ username: "user", password: "", "role": Role.Technician })).statusCode).toBe(400)
        
        // Fails for invalid role
        expect((await request(app.getHttpServer())
            .post("/user")
            .send({ username: "user", password: "password", "role": "" })).statusCode).toBe(400)
        ...
        const createManagerUserResponse: {
            ...
            .post("/user")
            .send(managerUserDetails)).body
        
        expect(createManagerUserResponse.username).toEqual(managerUserDetails.username)
        ...
        const createTech1UserResponse: {
            ...
            .post("/user")
            .send(tech1UserDetails)).body
        
        expect(createTech1UserResponse.username).toEqual(tech1UserDetails.username)
        ...
        const createTech2UserResponse: {
            ...
            .post("/user")
            .send(tech2UserDetails)).body
        
        expect(createTech2UserResponse.username).toEqual(tech2UserDetails.username)
        ...
        expect((await request(app.getHttpServer())
            .post("/auth/login")
            .send({ "username": "tech1", "password": "" })).statusCode).toBe(401)

        // Login all users
        managerUserDetails.access_token = (await request(app.getHttpServer())
            .post("/auth/login")
            .send(managerUserDetails)).body.access_token

        tech1UserDetails.access_token = (await request(app.getHttpServer())
            .post("/auth/login")
            .send(tech1UserDetails)).body.access_token

        tech2UserDetails.access_token = (await request(app.getHttpServer())
            .post("/auth/login")
            .send(tech2UserDetails)).body.access_token
        ...
        mockTask1.id = (await request(app.getHttpServer())
            .post("/task")
        ...
        mockTask2.id = (await request(app.getHttpServer())
            .post("/task")
        ...
        // Get tasks
        let managerTaskList = (await request(app.getHttpServer())
            .get("/task")
            .auth(managerUserDetails.access_token, { type: "bearer" })
            .send()).body

        let tech1TaskList = (await request(app.getHttpServer())
            .get("/task")
            .auth(tech1UserDetails.access_token, { type: "bearer" })
            .send()).body
        
        let tech2TaskList = (await request(app.getHttpServer())
            .get("/task")
        ...
        const updatedSummaryTask1 = (await request(app.getHttpServer())
            .patch(`/task/${mockTask1.id}/summary`)
            .auth(tech1UserDetails.access_token, { type: "bearer" })
            .send({
                summary: "New task 1 summary"
        ...
        // Complete task 2
        const completeTask2 = (await request(app.getHttpServer())
            .post(`/task/${mockTask2.id}/complete`)
            .auth(managerUserDetails.access_token, { type: "bearer" })
            .send()).body
        const todayAtMidnight = new Date()
        todayAtMidnight.setHours(0, 0, 0, 0)
        expect(new Date(completeTask2.performedAt).getTime()).toBeCloseTo(todayAtMidnight.getTime())
        ...
        managerTaskList = (await request(app.getHttpServer())
            .get("/task")
            .auth(managerUserDetails.access_token, { type: "bearer" })
            .send()).body

        tech1TaskList = (await request(app.getHttpServer())
            .get("/task")
            .auth(tech1UserDetails.access_token, { type: "bearer" })
            .send()).body
        
        tech2TaskList = (await request(app.getHttpServer())
            .get("/task")
            .auth(tech2UserDetails.access_token, { type: "bearer" })
            .send()).body
